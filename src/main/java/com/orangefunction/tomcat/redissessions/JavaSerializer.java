...
public class JavaSerializer implements Serializer {
	...
	public byte[] attributesHashFrom(RedisSession session) throws IOException {
		...
		for (Enumeration<String> enumerator = session.getAttributeNames(); enumerator
				.hasMoreElements();) {
	...
	@Override
	public byte[] serializeFrom(RedisSession session,
	...
	@Override
	public void deserializeInto(byte[] data, RedisSession session,
			SessionSerializationMetadata metadata) throws IOException,
			ClassNotFoundException {
		try (BufferedInputStream bis = new BufferedInputStream(
				new ByteArrayInputStream(data));
				ObjectInputStream ois = new CustomObjectInputStream(bis, loader);) {
			SessionSerializationMetadata serializedMetadata = (SessionSerializationMetadata) ois
					.readObject();
