...
public class RedisSessionManager extends ManagerBase implements Lifecycle {
	...
	public Session createSession(String requestedSessionId) {
		...
		try {
			...
			currentSessionSerializationMetadata
			...
			if (null != session) {
				try {
					...
					log.error("Error saving newly created session: "
							+ ex.getMessage());
	...
	public Session findSession(String id) throws IOException {
		...
		if (null == id) {
			...
			if (data != null) {
				DeserializedSessionContainer container = sessionFromSerializedData(
						id, data);
	...
	public DeserializedSessionContainer sessionFromSerializedData(String id,
			byte[] data) throws IOException {
		log.trace("Deserializing session " + id + " from Redis");

		if (Arrays.equals(NULL_SESSION, data)) {
			log.error("Encountered serialized session " + id
	...
	protected boolean saveInternal(Jedis jedis, Session session,
		...
		try {
			...
			SessionSerializationMetadata sessionSerializationMetadata = currentSessionSerializationMetadata
					.get();
			byte[] originalSessionAttributesHash = sessionSerializationMetadata
					.getSessionAttributesHash();
			...
			if (forceSave
					...
					|| !Arrays.equals(
							...
							(sessionAttributesHash = serializer
				...
				updatedSerializationMetadata
						.setSessionAttributesHash(sessionAttributesHash);
...
class DeserializedSessionContainer {
	...
	public DeserializedSessionContainer(RedisSession session,
			SessionSerializationMetadata metadata) {
